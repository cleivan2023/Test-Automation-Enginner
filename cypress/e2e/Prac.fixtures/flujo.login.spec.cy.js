describe('Visit te URL to login', () => {
    ...
    it('shoullog in with valid parems', () => {
        ...
        cy.fixture('usuarios.json').then((usuarios) => { //js busca este archivo y cuando termines ejecuta esto
        cy.get('input[type="email"]').type(usuarios.usuarioExistente.email);
        cy.get('input[type="password"]').type(usuarios.usuarioExistente.password);
        cy.get('#login_btn').click()
